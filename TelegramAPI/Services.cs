...
namespace TL
{
	...
	{
		public sealed partial class CollectorPeer : Peer, IPeerCollector
		{
			private readonly IDictionary<long, User> _users;
			private readonly IDictionary<long, ChatBase> _chats;

			public CollectorPeer(IDictionary<long, User> _users, IDictionary<long, ChatBase> _chats)
			{
				this._users = _users;
				this._chats = _chats;
			}
		...
		public static MessageEntity[] HtmlToEntities(this Client _, ref string text, bool premium = false, IReadOnlyDictionary<long, User> users = null)
		{
			...
			for (int offset = 0; offset < sb.Length;)
			{
				...
				if (c == '&')
				{
					...
					var html = System.Net.WebUtility.HtmlDecode(sb.ToString(offset, end - offset + 1));
					if (html.Length == 1)
					{
						sb[offset] = html[0];
				...
				else if (c == '<')
				{
					...
					switch (tag)
					{
						...
						default:
							...
							{
								tag = System.Net.WebUtility.HtmlDecode(tag[8..^1]);
