...
namespace WTelegram
{
	public class UpdateManager
	{
		...
		public IReadOnlyDictionary<long, MBoxState> State
		{
			get
			{
				_sem.Wait();
				try { return new Dictionary<long, MBoxState>(_local); }
		...
		private readonly List<(Update update, UpdatesBase updates, bool own, DateTime stamp)> _pending = new();
		...
		public UpdateManager(Client client, Func<Update, Task> onUpdate, IDictionary<long, MBoxState> state = null, IPeerCollector collector = null, bool reentrant = false)
		{
			_client = client;
			_onUpdate = onUpdate;
			_collector = collector ?? new Services.CollectorPeer(new Dictionary<long, User>(), new Dictionary<long, ChatBase>());
		...
		private async Task HandleUpdates(UpdatesBase updates, bool own)
		{
			...
			if (updates is UpdateShortSentMessage sent)
				updateList = new Update[] {new UpdateNewMessage { pts = sent.pts, pts_count = sent.pts_count, message = new Message {
		...
		private async Task HandleDifference(MessageBase[] new_messages, EncryptedMessageBase[] enc_messages, Updates_State state, UpdatesCombined updates)
		{
			...
			try
			{
				...
				{
					var update = state == null 
						? new UpdateNewChannelMessage() 
						: new UpdateNewMessage() { pts = state.pts, pts_count = 1 };
		...
		public void SaveState(string statePath)
			=> System.IO.File.WriteAllText(statePath, JsonConvert.SerializeObject(State, Helpers.JsonOptions));
		public static Dictionary<long, MBoxState> LoadState(string statePath) => !System.IO.File.Exists(statePath) ? null
			: JsonConvert.DeserializeObject<Dictionary<long, MBoxState>>(System.IO.File.ReadAllText(statePath), Helpers.JsonOptions);
