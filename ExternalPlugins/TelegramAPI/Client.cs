...
namespace WTelegram
{
...
#endif
	{
		...
		public Client(Func<string, string> configProvider = null, Stream sessionStore = null)
		{
			...
			Helpers.Log(1, $"WTelegramClient {1} running under {System.Runtime.InteropServices.RuntimeInformation.FrameworkDescription}");
		}
		...
		{
			"session_pathname" => Path.Combine(Application.persistentDataPath, "WTelegram.session"),
#if DEBUG
			"server_address" => "2>149.154.167.40:443",	// Test DC 2
#else
			"server_address" => "2>149.154.167.50:443",	// DC 2
#endif
			"device_model" => "It's not your business",
			"system_version" => "It's not your business",
			"app_version" => "It's not your business",
		...
		private static string AskConfig(string config)
		{
			...
			{
				Debug.Log("Welcome! You can obtain your api_id/api_hash at https://my.telegram.org/apps");
				return null;
			}
			...
			Debug.Log($"Enter {config.Replace('_', ' ')}: "); 
			return string.Empty;
		}
		...
		private async Task DoConnectAsync(bool quickResume)
		{
			...
			try
			{
				...
				else
				{
					...
					var initParams = JSONValue.FromJToken(JObject.Parse(Config("init_params")));
...
#pragma warning restore CS0618
		...
		private async Task SendAsync(IObject msg, bool isContent, Rpc rpc = null)
		{
			...
			if (isContent)
			{
				...
				if (_httpWait != null && NewMsgId(false) is var (hwId, hwSeqno))
					(messages ??= new()).Add(new(hwId, hwSeqno, _httpWait));
				if (CheckMsgsToAck() is MsgsAck msgsAck && NewMsgId(false) is var (ackId, ackSeqno))
					(messages ??= new()).Add(new(ackId, ackSeqno, msgsAck));
				if (messages != null)
				{
		...
		public async Task HttpWait(HttpWait httpWait = null)
		{
			...
			var container = new MsgContainer { messages = new() };
