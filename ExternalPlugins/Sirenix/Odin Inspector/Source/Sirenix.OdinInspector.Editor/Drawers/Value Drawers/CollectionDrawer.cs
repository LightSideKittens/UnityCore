...
#pragma warning disable
    ...
    public class CollectionDrawer<T> : OdinValueDrawer<T>, IDefinesGenericMenuItems, IHackyListDrawerInteractions
    {
        ...
        {
            this.selectorSettings = this.Property.GetAttribute<TypeSelectorSettingsAttribute>();
            ...
            if (customListDrawerOptions.DefaultExpandedStateHasValue)
                this.Property.State.Expanded = customListDrawerOptions.DefaultExpandedState;
            ...
            {
                this.info.ElementColor = ValueResolver.Get<Color>(
                    this.Property,
                    this.info.CustomListDrawerOptions.ElementColor,
                    new ValueResolvers.NamedValue("index", typeof(int)),
                    new ValueResolvers.NamedValue("defaultColor", typeof(Color)));
            }
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            this.info.Label.image = label?.image;
        ...
        private DropZoneHandle BeginDropZone()
        {
            ...
            var dropZone = DragAndDropManager.BeginDropZone(GetKey(this.Property), this.info.CollectionResolver.ElementType, true);
        ...
        private void DrawToolbar()
        {
            SirenixEditorGUI.BeginHorizontalToolbar();
            {
                ...
                bool drawFoldout = this.info.CustomListDrawerOptions.ShowFoldout;
                ...
                {
                    if (OdinObjectSelector.IsReadyToClaim(this.info, CollectionDrawerId))
                    {
                        object[] claimedObjects = OdinObjectSelector.ClaimMultiple(this.info.Property.Tree.WeakTargets.Count);

                        if (this.onValuesCreated != null)
                        {
                            this.onValuesCreated(claimedObjects);
                            this.onValuesCreated = null;
                        }
                        else
                        {
                            this.info.CollectionResolver.QueueAdd(claimedObjects);
                        }
                    }
        ...
        private void StartCreatingValues(Rect potentialPopupPosition)
        {
            ...
            else
            {
                ...
                if (wasFallback)
                {
                    Type elementType = this.info.CollectionResolver.ElementType;

                    if (elementType == typeof(Type))
                    {
                        var typeSelectorSettings = this.Property.GetAttribute<TypeDrawerSettingsAttribute>();

                        List<Type> validTypes;

                        if (typeSelectorSettings != null)
                        {
                            if (typeSelectorSettings.BaseType != null)
                            {
                                validTypes = TypeRegistry.GetInheritors(typeSelectorSettings.BaseType);
                            }
                            else
                            {
                        ...
                        var selector = new TypeSelectorV2(validTypes);
                        ...
                        selector.SelectionConfirmed += types =>
                        {
                            ...
                            int weakTargetCount = this.info.Property.Tree.WeakTargets.Count;

                            var weakTargets = new Type[weakTargetCount];

                            for (var i = 0; i < weakTargetCount; i++)
                            {
                                weakTargets[i] = type;
                            }
                            ...
                            if (this.onValuesCreated != null)
                            {
                                this.onValuesCreated(weakTargets);
                                this.onValuesCreated = null;
                            }
                            else
                            {
                                this.info.CollectionResolver.QueueAdd(weakTargets);
                            }
                        };
                    ...
                    {
                        bool allowSceneObjects = InspectorPropertyInfoUtility.InspectorPropertySupportsAssigningSceneReferences(this.Property);

                        OdinObjectSelector.Show(potentialPopupPosition,
                                                this.info,
                                                CollectionDrawerId,
                                                null,
                                                elementType,
                                                disallowNullValues: !this.Property.ValueEntry.SerializationBackend.SupportsPolymorphism,
        ...
        private void DrawItems()
        {
            ...
            var rect = SirenixEditorGUI.BeginVerticalList();
            {
                ...
                {
                    var dragHandle = this.BeginDragHandle(j);
                    {
                        if (drawEmptySpace)
                        {
                        ...
                        if (drawEmptySpace)
                        {
                            ...
                            {
                                var targetRect = GUILayoutUtility.GetRect(0, height);
                                drawEmptySpace = false;
        ...
        private object GetKey(InspectorProperty prop) => prop;

        private DragHandle BeginDragHandle(int j)
        {
            var child = this.info.FilteredChildren[j];
            var dragHandle = DragAndDropManager.BeginDragHandle(GetKey(child), child.ValueEntry.WeakSmartValue, this.info.IsReadOnly ? DragAndDropMethods.Reference : DragAndDropMethods.Move);
        ...
        private Rect DrawItem(InspectorProperty itemProperty, DragHandle dragHandle, Color evenColor, Color oddColor)
        {
            ...
            rect = SirenixEditorGUI.BeginListItem(false, this.info.ListItemStyle, color, color, color, color, listItemOptions);
            {
                ...
                {
                    if (SirenixEditorGUI.SDFIconButton(listItemInfo.Value.RemoveBtnRect, (GUIContent)null, SdfIconType.X, style: SirenixGUIStyles.IconButton))
                    {
                        if (this.info.OrderedCollectionResolver != null)
                        {
                            if (index >= 0)
