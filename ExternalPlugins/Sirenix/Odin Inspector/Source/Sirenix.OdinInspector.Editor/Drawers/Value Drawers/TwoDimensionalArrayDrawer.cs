...
#pragma warning restore 1591 // Missing XML comment for publicly visible type or member
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            if (context == null || colCount != context.ColCount || rowCount != context.RowCount)
            {
                ...
                context.LabelGetter = ValueResolver.Get<(string, LabelDirection)>(this.Property, context.Attribute.Labels, new[]
                {
        ...
        private static void SetSquareRowHeights(Context context)
        {
            if (context.ColCount == 1)
            {
                ...
                for (var y = 0; y < context.RowCount; y++)
                {
                    int _y = context.Table.RowCount - 1 - y;

                    for (var x = 0; x < context.Table.ColumnCount; x++)
                    {
        ...
        private void DrawRows(Rect rect, IPropertyValueEntry<TArray> entry, Context context, int rowIndex)
        {
            if (rowIndex < context.RowCount)
            {
                ...
                else
                {
                    ...
                    (rect.width, rect.height) = (height, width);
                    var oldMatrix = GUI.matrix;

                    if (rowLabelDirection == LabelDirection.TopToBottom)
                    {
                        rect = rect.AddX(width);
                        GUIUtility.RotateAroundPivot(90, rect.position);
                    }
                    else if (rowLabelDirection == LabelDirection.BottomToTop)
                    {
                        rect = rect.AddY(height);
                        GUIUtility.RotateAroundPivot(270, rect.position);
                    }
        ...
        private void DrawColumn(Rect rect, IPropertyValueEntry<TArray> entry, Context context, int columnIndex)
        {
            if (columnIndex < context.ColCount)
            {
                ...
                var (label, _) = context.LabelGetter.GetValue();
        ...
        where TElement : UnityEngine.Object
    {
        ...
        {
            this.isAssetOnly = this.ValueEntry.Property.GetAttribute<AssetsOnlyAttribute>() != null;
        }

        protected override TElement DrawElement(Rect rect, TElement value)
        {
            bool editable = !this.TableMatrixAttribute.IsReadOnly;
            value = SirenixEditorFields.PreviewObjectField(rect, value, false, editable, editable, !this.isAssetOnly);
            return value;
        }
