...
#pragma warning restore 0618
        ...
        private static readonly TypeGroup ScriptTypesRootGroup = new TypeGroup("Script Types");
        private static readonly TypeGroup ImportedAssemblyTypesRootGroup = new TypeGroup("Imported Assembly Types");
            ...
            public void UpdateSearch(string search, DisplayType displayType)
            {
                ...
                for (int i = 0; i < this.SubTypes.Count; i++)
                {
                    ...
                    if ((displayType == DisplayType.AllScriptableObjects && !typeof(ScriptableObject).IsAssignableFrom(type))
                        ...
                        || (displayType == DisplayType.UserScripts && !((AssemblyUtilities.GetAssemblyCategory(type.Assembly) & AssemblyCategory.ProjectSpecific) != 0 )))
                    {
        ...
        {
            var assemblyType = AssemblyUtilities.GetAssemblyCategory(type.Assembly);
            ...
            switch (assemblyType)
            {
                case AssemblyCategory.Scripts:
                    group = ScriptTypesRootGroup;
                    break;

                case AssemblyCategory.ImportedAssemblies:
                    group = ImportedAssemblyTypesRootGroup;
                    break;
                ...
                // If we hit the default case, something actually went wrong.
        ...
        {
            return GetEditorsForCompilation(ScriptTypesRootGroup)
                .AppendWith(GetEditorsForCompilation(ImportedAssemblyTypesRootGroup))
        ...
        private void DrawRootTypeGroup(InspectorDefaultEditors editorCategory, IPropertyValueEntry<InspectorTypeDrawingConfig> entry, string searchText)
        {
            ...
            switch (editorCategory)
            {
                case InspectorDefaultEditors.UserTypes:
                    typeGroup = ScriptTypesRootGroup;
                    break;

                case InspectorDefaultEditors.PluginTypes:
                    typeGroup = ImportedAssemblyTypesRootGroup;
                    break;
