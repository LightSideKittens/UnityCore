...
#pragma warning restore CS0414
        ...
        private static DateTime LastScanTime
        {
            get
            {
                if (!hasInitializedLastScanTime)
                {
                    ...
                    try
                    {
                        var dateLongStr = EditorPrefs.GetString("OdinSerializer_AOTGenerationConfig_LastScan", "0");

                        if (long.TryParse(dateLongStr, out long dateLong))
                        {
            ...
            set
            {
                if (value != lastScanTime)
                {
                    ...
                    EditorPrefs.SetString("OdinSerializer_AOTGenerationConfig_LastScan", binary.ToString("D"));
                }
            }
        }
        ...
        private void GenericVariantsTitleGUI()
        {
            ...
            GUILayout.Label("Last scan: " + LastScanTime.ToString(), SirenixGUIStyles.CenteredGreyMiniLabel);
        ...
        [DisableIf("EditorOnlyBuild")]
        [Button("Scan Project", 30), HorizontalGroup("ButtonMargin", 0.2f, PaddingRight = -4), PropertyOrder(2)]
        ...
        {
            SirenixEditorGUI.MessageBox("Scanning the entire project might take a while. It will scan the entire project for relevant types including " +
                "ScriptableObjects, prefabs and scenes. Modified type entries will not be touched.", MessageType.Warning);
        }
