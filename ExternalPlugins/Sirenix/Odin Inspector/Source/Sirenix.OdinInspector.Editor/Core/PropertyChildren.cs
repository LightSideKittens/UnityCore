...
#pragma warning disable
    ...
    public sealed class PropertyChildren : IEnumerable<InspectorProperty>
    {
        ...
        internal void ClearAndDisposeChildrenFromIndex(int index)
        {
            ...
            foreach (var childIndex in toRemoveIndices)
            {
                this.infosByIndex.Remove(childIndex);
                this.childrenByIndex.Remove(childIndex);
                this.pathsByIndex.Remove(childIndex);
            }
        }
        ...
        public void Update()
        {
            ...
            {
                var childCountBefore = this.property.ChildResolver.ChildCount;
                this.property.ChildResolver.ForceUpdateChildCount();
                var childCountAfter = this.property.ChildResolver.ChildCount;

                if (childCountAfter < childCountBefore)
                {
                    this.ClearAndDisposeChildrenFromIndex(childCountAfter);
                }
            }
        }
