...
#pragma warning disable
    ...
    public sealed class InspectorProperty : IDisposable
    {
        ...
        public IPropertyValueEntry<T> TryGetTypedValueEntry<T>()
        {
            ...
            if (!typeof(T).IsAssignableFrom(this.ValueEntry.TypeOfValue)) return null;
        ...
        public void Draw(GUIContent defaultLabel)
        {
            ...
            try
            {
                ...
                if (show)
                {
                    ...
                    var measureBox = e == EventType.Repaint;
        ...
        private void UpdateValueEntry()
        {
            ...
            if (!this.Info.TypeOfValue.IsValueType)
            {
                ...
                {
                    if (this.ValueEntry == null 
                        || (this.ValueEntry.IsAlias && (this.ValueEntry as PropertyValueEntryAlias).TValueGenericTypeArgument != containedType) 
                        || (this.ValueEntry.IsAlias == false && this.ValueEntry.TypeOfValue != this.ValueEntry.BaseValueType))
                    {
                        this.DisposeExistingSetup();
