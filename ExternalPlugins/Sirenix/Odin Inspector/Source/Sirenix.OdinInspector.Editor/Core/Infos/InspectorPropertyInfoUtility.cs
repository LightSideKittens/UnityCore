...
#pragma warning disable
    ...
    public static class InspectorPropertyInfoUtility
    {
        ...
        private static Type UnityEngine_EditorWindow_Type = typeof(UnityEditor.EditorWindow);
        ...
        public static List<InspectorPropertyInfo> CreateMemberProperties(InspectorProperty parentProperty, Type type, bool includeSpeciallySerializedMembers)
        {
            ...
            var assemblyFlag = AssemblyUtilities.GetAssemblyCategory(type.Assembly);

            bool isUnityType = assemblyFlag == AssemblyCategory.UnityEngine;
        ...
        private static void FindUnityProperties(InspectorProperty parentProperty, Type type, List<InspectorPropertyInfo> result)
        {
            ...
            try
            {
                ...
                else if (UnityVersion.IsVersionOrGreater(2017, 1))
                {
                    ...
                    GameObject go = new GameObject("Odin_EmittedUnityPropertyExtractor_Temp (If you see this in the hierarchy, something went badly wrong)");
                    toDestroy = go;

                    try
                    {
                        handle = UnityPropertyEmitter.CreateEmittedMonoBehaviourProperty("InspectorPropertyInfo_UnityPropertyExtractor", type, 1, ref go);
        ...
        private static void PopulateMemberInspectorProperties(InspectorProperty parentProperty, Type type, bool includeSpeciallySerializedMembers, List<InspectorPropertyInfo> properties)
        {
            ...
            if (!object.ReferenceEquals(baseType, null) &&
                ...
                !object.ReferenceEquals(baseType, UnityEngine_EditorWindow_Type) &&
            ...
            //{
            //    Debug.Log("What the, this thing had >100 members: " + type.GetNiceFullName() + " - Path: " + parentProperty.Path + " - Root: " + parentProperty.Tree.TargetType.GetNiceFullName() + " - Base Value: " + parentProperty.ValueEntry.TypeOfValue.GetNiceFullName());
            //}
        ...
        public static bool InspectorPropertySupportsAssigningSceneReferences(InspectorProperty property)
        {
            ...
            // If property is Collection, then check the ElementType
            if (property.ChildResolver is ICollectionResolver collectionResolver)
            {
                valueType = collectionResolver.ElementType;
            }
            else if (property.ChildResolver is IOrderedCollectionResolver orderedCollectionResolver)
            {
                valueType = orderedCollectionResolver.ElementType;
            }
            else
            {
                valueType = property.ValueEntry.BaseValueType;
            }

            bool valueTypeCanHoldUnitySceneObjects =
                valueType.IsInterface ||
                valueType == typeof(GameObject) ||
                typeof(UnityEngine.Component).IsAssignableFrom(valueType) ||
                valueType == typeof(UnityEngine.Object) ||
                valueType == typeof(System.Object);

            if (!valueTypeCanHoldUnitySceneObjects)
            {
                return false;
            }

            // If the serialization backend is none, always allow scene objects.
            if (property.ValueEntry != null && property.ValueEntry.SerializationBackend == SerializationBackend.None)
            {
                allowSceneObjects = true;
            }
            // If the serialization root is a non Unity object, always allow scene objects.
            else if (root.ValueEntry != null && !typeof(UnityEngine.Object).IsAssignableFrom(root.ValueEntry.TypeOfValue))
            {
                allowSceneObjects = true;
            }
            // If the serialization root is a component and the scene is a scene and not a prefab stage.
            else if (root.ValueEntry != null && (root.ValueEntry.WeakSmartValue is UnityEngine.Object uObj))
            {
                ...
                if (uObj is GameObject)
                    ...
                    go = ((Component)uObj).gameObject;
                else
                    go = null;
                
                if (go != null)
                {
                    var kind = OdinPrefabUtility.GetPrefabKind(go);
                    
                    if (kind == PrefabKind.None || kind == PrefabKind.NonPrefabInstance || kind == PrefabKind.PrefabInstance || kind == PrefabKind.InstanceInScene)
                    {
                        allowSceneObjects = true;
                    }
                    else 
                    {
                        allowSceneObjects = OdinPrefabUtility.IsPartOfPrefabStage(go);
                    }
                }
                else 
                {
                    // Not sure what kind of unity object this is, so we allow scene objects.
                    allowSceneObjects = true;
                }
            }
            else if (root.ValueEntry != null && root.ValueEntry.WeakSmartValue is UnityEngine.GameObject go)
            {
                allowSceneObjects = go != null && OdinPrefabUtility.IsPartOfPrefabStage(go);
            }
            else
            {
            ...
            // Prevent scene objects if the property has the AssetsOnly attribute
