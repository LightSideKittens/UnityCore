        ...
        public override void ProcessSelfAttributes(InspectorProperty property, List<Attribute> attributes)
        {
            ...
            if (isBaseType)
            {
                ...
                while (true)
                {
                    ...
                    var flag = AssemblyUtilities.GetAssemblyCategory(current.Assembly);
                    if ((flag & AssemblyCategory.ProjectSpecific) == 0) break; // Must be a user-defined/imported type
            ...
            else
            {
                while (true)
                {
                    if (current == null) break;
                    var flag = AssemblyUtilities.GetAssemblyCategory(current.Assembly);
                    if ((flag & (AssemblyCategory.Scripts | AssemblyCategory.ImportedAssemblies)) == 0) break; // Must be a user-defined/imported type
