...
#pragma warning disable
    ...
    public static class DrawerUtilities
    {
        ...
        {
            DecoratorDrawerCreatePropertyGUIMethod = typeof(DecoratorDrawer).GetMethod("CreatePropertyGUI", BindingFlags.Public | BindingFlags.Instance);
                    ...
                    if (isPropertyDrawer)
                    {
                        ...
                        else
                        {
                            if (useForChildren || drawnType.IsAbstract)
                            {
                                ...
                                {
                                    wrapper = typeof(AbstractTypeUnityPropertyDrawer<,,>).MakeGenericType(type, drawnType, drawnType);
								}
                                else
                                {
        ...
        private static DrawerPriority CalculateDrawerPriority(Type drawerType)
        {
            ...
            // Find a DrawerPriorityAttribute if there is one anywhere and use the priority from that
            {
                ...
                if (DrawerIsUnityAlias(drawerType))
                {
                    ...
                    {
                        var flag = AssemblyUtilities.GetAssemblyCategory(innerDrawer.Assembly);
                        var drawnType = drawerArgs[1]; // Value or attribute, depending

                        bool isUnityDrawer = (flag & AssemblyCategory.UnityEngine) != 0;
