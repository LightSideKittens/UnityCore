...
#pragma warning disable
    ...
    public class PropertySearchFilter
    {
        ...
        public virtual bool IsMatch(InspectorProperty property, string searchTerm)
        {
            ...
            bool isEditableKeyValuePair = property.ParentType.IsGenericType &&
                                          property.ParentType.GetGenericTypeDefinition() == typeof(EditableKeyValuePair<,>);

            if (!isEditableKeyValuePair) 
            {
            ...
            {
                if (this.HasSearchFlag(SearchFilterOptions.TypeOfValue))
                {
                    if (TypeRegistry.TryGetCustomName(property.ValueEntry.TypeOfValue, out string customName))
                    {
                        if (stringMatcher(searchTerm, customName))
                        {
                            return true;
                        }
                    }
