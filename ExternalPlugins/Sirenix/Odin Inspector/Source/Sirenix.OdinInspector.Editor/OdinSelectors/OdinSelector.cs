...
#pragma warning disable
    ...
    public abstract class OdinSelector<T> : ISelectionChangeListener
    {
        ...
        {
            this.SelectionTree.Config.SelectMenuItemsOnMouseDown = true;
        ...
        private void SetupWindow(OdinEditorWindow window, EditorWindow prevSelectedWindow)
        {
            ...
            window.OnBeginGUI += () =>
            {
                ...
                {
                    // NOTE: Since this window is an auxiliary window, it will automatically close when it loses focus.
                    if (prevSelectedWindow)
                    {
                        prevSelectedWindow.Focus();
                    }
        ...
        internal static bool DrawSelectorButton<TSelector>(Rect buttonRect, string label, SdfIconType valueIcon, GUIStyle style, int id, bool returnValuesOnSelectionChange, out Action<TSelector> bindSelector, out Func<IEnumerable<T>> resultGetter)
        ...
        internal static bool DrawSelectorButton<TSelector>(Rect buttonRect, GUIContent label, SdfIconType valueIcon, GUIStyle style, int id, bool returnValuesOnSelectionChange, out Action<TSelector> bindSelector, out Func<IEnumerable<T>> resultGetter)
            where TSelector : OdinSelector<T>
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                var showIsDown = GUIUtility.hotControl == id || GUIHelper.focusedControlId == id;
                style = style ?? EditorStyles.popup;


                var s = EditorGUIUtility.GetIconSize();
                EditorGUIUtility.SetIconSize(new Vector2(16, 16));
                if (valueIcon != SdfIconType.None)
                {
                    style.Draw(buttonRect, GUIContent.none, showIsDown, showIsDown, false, GUIUtility.keyboardControl == id);
                    SdfIcons.DrawIcon(buttonRect.TakeFromLeft(buttonRect.height * 1.2f).Padding(2, 0, 3, 3), valueIcon);
        ...
        {
            return DrawSelectorDropdown(rect, btnLabel, createSelector, true, style);
        }
        ...
        public static IEnumerable<T> DrawSelectorDropdown(Rect rect, GUIContent btnLabel, Func<Rect, OdinSelector<T>> createSelector, bool returnValuesOnSelectionChange, GUIStyle style = null, SdfIconType valueIcon = SdfIconType.None)
        {
            ...
            style = style ?? EditorStyles.popup;
            if (DrawSelectorButton(rect, tmpValueLabel, valueIcon, style, id, returnValuesOnSelectionChange, out bindSelector, out getResult))
            {
        ...
        {
            return DrawSelectorDropdown(label, btnLabel, createSelector, true, style, options);
        }
        ...
        public static IEnumerable<T> DrawSelectorDropdown(GUIContent label, GUIContent btnLabel, Func<Rect, OdinSelector<T>> createSelector, bool returnValuesOnSelectionChange, GUIStyle style = null, params GUILayoutOption[] options)
        {
            ...
            style = style ?? EditorStyles.popup;
            if (DrawSelectorButton(rect, tmpValueLabel, SdfIconType.None, style, id, returnValuesOnSelectionChange, out bindSelector, out getResult))
            {
                var selector = createSelector(rect);
                bindSelector(selector);
