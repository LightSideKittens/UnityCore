...
#pragma warning disable
    ...
    public class OdinMenuItem
    {
        ...
        public OdinMenuItem NextSelectableMenuItem
        {
            get
            {
                OdinMenuItem next = this.NextVisualMenuItem;
        ...
        public OdinMenuItem PrevSelectableMenuItem
        {
            get
            {
                OdinMenuItem prev = this.PrevVisualMenuItem;
        ...
        public virtual bool Toggled
        {
            ...
            set
            {
                ...
                if (this.menuTree.Config.UseCachedExpandedStates)
                {
                    ...
                    hasChanged = this.isToggledContext.Value != value;

                    this.isToggledContext.Value = value;
                }
                else
                {
                    hasChanged = this.nonCachedToggledState != value;
                    
                    this.nonCachedToggledState = value;
                }

                if (this.menuTree.Config.EXPERIMENTAL_INTERNAL_SparseFixedLayouting && hasChanged)
                {
                    this.menuTree.MarkLayoutChanged();
                }
            }
        }
        ...
        public virtual void DrawMenuItem(int indentLevel)
        {
            ...
            if (eType == EventType.Repaint)
            {
                ...
                // Hover
                if (this.IsSelectable && this.IsEnabled && !selected && this.rect.Contains(e.mousePosition))
                {
                ...
                if (this.ChildMenuItems.Count > 0 && !this.MenuTree.DrawInSearchMode && this.Style.DrawFoldoutTriangle)
                {
                    ...
                    EditorIcon icon = this.Toggled ? EditorIcons.TriangleDown : EditorIcons.TriangleRight;
                ...
                if (this.SdfIcon != SdfIconType.None)
                {
                ...
                if (hasIcon)
                {
                    if (texIcon)
                    {
                        ...
                        if (!selected) GUIHelper.PushColor(new Color(1, 1, 1, this.Style.NotSelectedIconAlpha));
                        ...
                        if (!selected) GUIHelper.PopColor();
                    }
                    else
                    {
                        var iconRect = this.labelRect.AlignLeft(this.Style.IconSize - 3).AlignMiddle(this.Style.IconSize - 3);
                        iconRect.x += this.Style.IconOffset;
                        if (selected)
                            SdfIcons.DrawIcon(iconRect, this.SdfIcon, new Color(1, 1, 1, 1), this.style.SelectedColor);
                        else
                        {
                            var col = this.SdfIconColor ?? SirenixGUIStyles.HighlightedTextColor;
                            col.a = this.style.NotSelectedIconAlpha;
                            SdfIcons.DrawIcon(iconRect, this.SdfIcon, col);
                        }
                    }
            ...
            if (!this.IsEnabled)
            {
                EditorGUI.DrawRect(this.rect, new Color(0, 0, 0, 0.15f));

                if (this.menuTree.Selection.SelectedValue == this)
                {
                    this.menuTree.Selection.Clear();
                }
            }
            ...
            {
                if (this.menuTree.Selection.SupportsMultiSelect)
                {
                    foreach (object itemValue in this.menuTree.Selection.SelectedValues)
                    {
                ...
                if (this.menuTree.Selection.SelectedValue == this.Value)
                {
                    this.menuTree.Selection.Clear();
                }
            }
        ...
        internal int CountVisibleRecursively()
        {
            ...
            foreach (OdinMenuItem childMenuItem in this.ChildMenuItems)
            {
        ...
        {
            OdinGUIScrollView scrollView = this.menuTree.ScrollView;

            scrollView.AllocateRect(this.menuTree.Config.DefaultMenuStyle.Height, this);

            if (this.menuTree.DrawInSearchMode || !this.Toggled)
            {
                return;
            }

            ++scrollView.Indentation;
        ...
        protected void HandleMouseEvents(Rect rect, Rect triangleRect)
        {
            if (!this.IsEnabled || !this.IsSelectable)
            {
                if (Event.current.OnMouseDown(this.triangleRect, 0))
                {
                    ...
                    if (toggle && Event.current.IsMouseOver(this.triangleRect))
                    {
                        ...
                        bool state = !this.Toggled;

                        if (Event.current.modifiers == EventModifiers.Alt)
                        {
                            foreach (OdinMenuItem item in this.GetChildMenuItemsRecursive(true))
                            {
                                item.Toggled = state;
                            }
                        }
            ...
            if (this.MenuTree.Config.SelectMenuItemsOnMouseDown)
            {
