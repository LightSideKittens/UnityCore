...
#pragma warning disable
    ...
    internal class MemberSerializationInfo
    {
        ...
        private MemberSerializationInfo(MemberInfo member, string[] notes, SerializationFlags flags, SerializationBackendFlags serializationBackend)
        {
            ...
            // Handle special case for SerializeReference
            if (this.Info.HasAll(SerializationFlags.OdinSerializeAttribute | SerializationFlags.NonSerializedAttribute | SerializationFlags.SerializeReferenceAttribute))
            {
                this.OdinMessageType  = InfoMessageType.Warning;
                this.UnityMessageType = InfoMessageType.Warning;
            }
            ...
            // Is the use of SerializeReference going against what's specified in Unity's documentation?
            if (this.Info.HasAll(SerializationFlags.SerializeReferenceAttribute | SerializationFlags.SerializedByUnity))
            {
                Type memberType = member.GetReturnType();

                SerializeReferenceValidityResult serializeRefValidity = SerializeReferenceUtility.ValidateBaseType(memberType, out string _);

                switch (serializeRefValidity)
                {
        ...
        public static List<MemberSerializationInfo> CreateSerializationOverview(Type type, SerializationBackendFlags serializationBackend, bool includeBaseTypes)
        {
            ...
            return type.GetAllMembers(includeBaseTypes ? Flags.InstanceAnyVisibility : Flags.InstanceAnyDeclaredOnly)
                ...
                .Where(x => (AssemblyUtilities.GetAssemblyCategory(x.DeclaringType.Assembly) & AssemblyCategory.UnityEngine) == 0) // Exclude members from UnityEngine types
        ...
        private static MemberSerializationInfo CreateInfoFor(MemberInfo member, SerializationBackendFlags serializationBackend, bool serializeUnityFields, ISerializationPolicy serializationPolicy)
        {
            ...
            // Does the member have a SerializeReference attribute?
        ...
        private static string[] CreateNotes(MemberInfo member, SerializationFlags flags, SerializationBackendFlags serializationBackend, string serializationPolicyId)
        {
            ...
            if (serializationBackend.HasNone(SerializationBackendFlags.Odin) || flags.HasAll(SerializationFlags.DefaultSerializationPolicy))
            {
                ...
                if (flags.HasAny(SerializationFlags.SerializedByOdin | SerializationFlags.SerializedByUnity))
                {
                    ...
                    // Why?
                    var relevant = flags & (SerializationFlags.Public | SerializationFlags.SerializeFieldAttribute | SerializationFlags.NonSerializedAttribute | SerializationFlags.SerializeReferenceAttribute);
                    ...
                    switch (relevant)
                    {
                        ...
                        case SerializationFlags.SerializeReferenceAttribute | SerializationFlags.OdinSerializeAttribute:
                            buffer.Append("the [SerializeReference] and [OdinSerialize] attributes are defined.");
                            break;
                        
                        case SerializationFlags.Public | SerializationFlags.SerializeReferenceAttribute | SerializationFlags.OdinSerializeAttribute:
                            buffer.Append("the [SerializeReference] and [OdinSerialize] attributes are defined");
                        ...
                        case SerializationFlags.OdinSerializeAttribute | SerializationFlags.NonSerializedAttribute | SerializationFlags.SerializeFieldAttribute:
                        ...
                        case SerializationFlags.Public | SerializationFlags.OdinSerializeAttribute | SerializationFlags.NonSerializedAttribute | SerializationFlags.SerializeFieldAttribute:
                    ...
                    if (serializationBackend.HasAll(SerializationBackendFlags.Unity) && flags.HasNone(SerializationFlags.SerializedByUnity))
                    {
                        ...
                        else if ((flags & SerializationFlags.SerializeReferenceAttribute) != SerializationFlags.SerializeReferenceAttribute &&
                                 UnitySerializationUtility.GuessIfUnityWillSerialize(member.GetReturnType()) == false)
                        {
                        ...
                        else if (flags.HasAny(SerializationFlags.Public | SerializationFlags.SerializeFieldAttribute | SerializationFlags.SerializeReferenceAttribute) == false)
                        {
                ...
                else // Why not?
                {
                    ...
                    // Using both [SerializeField] and [NonSerialized] attributes.
                    if (flags.HasAll(SerializationFlags.SerializeReferenceAttribute | SerializationFlags.NonSerializedAttribute) && flags.HasNone(SerializationFlags.OdinSerializeAttribute))
                    {
            ...
            if (serializationBackend.HasAll(SerializationBackendFlags.UnityAndOdin)) {
                bool isUnitySerializedProperty = flags.HasAll(SerializationFlags.SerializedByUnity | SerializationFlags.Property);

                if (flags.HasAll(SerializationFlags.SerializedByOdin | SerializationFlags.SerializeReferenceAttribute | SerializationFlags.TypeSupportedByUnity) && !isUnitySerializedProperty)
                {
            ...
            if (serializationBackend.HasAll(SerializationBackendFlags.Unity) && flags.HasAll(SerializationFlags.SerializeReferenceAttribute))
            {
                ...
                SerializeReferenceValidityResult serializeRefValidity = SerializeReferenceUtility.ValidateBaseType(memberType, out string errorMessage);

                if (serializeRefValidity != SerializeReferenceValidityResult.Valid)
                {
    ...
    internal enum SerializationFlags
    {
        ...
        SerializeReferenceAttribute = 1 << 12,
    }
