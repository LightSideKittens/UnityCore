        ...
        private void DrawTopBar()
        {
            Rect rect = SirenixEditorGUI.BeginHorizontalToolbar();
            {
                ...
                if (selectA || selectB)
                {
                    ...
                    var source = AssemblyUtilities.GetTypes(AssemblyCategory.ProjectSpecific)
                        .Where(x => !x.IsAbstract && x.IsClass && x.InheritsFrom<UnityEngine.Object>())
                        .Where(x => !x.Assembly.FullName.StartsWith("Sirenix"))
                        .OrderBy(x => AssemblyUtilities.GetAssemblyCategory(x.Assembly))
                        .OrderBy(x => AssemblyUtilities.GetAssemblyCategory(x.Assembly))
                        .ThenBy(x => x.Namespace)
                        .ThenByDescending(x => x.Name);

                    var p = TypeSelectorHandler_WILL_BE_DEPRECATED.InstantiateSelector(source, showNoneItem: false);
