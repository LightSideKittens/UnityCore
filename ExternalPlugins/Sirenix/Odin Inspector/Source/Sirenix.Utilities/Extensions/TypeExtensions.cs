...
#pragma warning disable
    ...
    public static class TypeExtensions
    {
		...
		/// <summary>
        /// <para>Determines if a given <see cref="Type"/> has a default constructor.</para>
		...
		/// </summary>
		/// <param name="self">The <see cref="Type"/> to investigate.</param>
		/// <returns><c>true</c> if a default constructor has been found; otherwise <c>false</c>.</returns>
        public static bool HasDefaultConstructor(this Type self) => self == typeof(string) ||
                                                                    self.IsArray ||
                                                                    self.IsValueType ||
                                                                    self.GetConstructor(Type.EmptyTypes) != null;

        /// <summary>
        /// Attempts to instantiate an object of a given <see cref="Type"/> with it's default constructor. If no default constructor is found then it attempts to find the most suitable alternative constructor and instantiate the <see cref="Type"/> with default parameters.
        /// </summary>
        /// <param name="type">The <see cref="Type"/> to instantiate.</param>
        /// <param name="preferUninitializedOverNonDefault">Determines if the use of <see cref="FormatterServices.GetUninitializedObject"/> is preferred over a non-default constructor call.</param>
        ...
        public static object InstantiateDefault(this Type type, bool preferUninitializedOverNonDefault)
		{
            if (type.IsInterface || type.IsAbstract)
            {
                ...
                throw new
                    ArgumentException($"Invalid argument received in parameter '{nameof(type)}', can't construct Interfaces or Abstracts ({typeDecl}).");
            }
			...
			var parametersValues = new object[ctorParams.Length];
			...
			for (var i = 0; i < parametersValues.Length; i++)
			{
                ...
                parametersValues[i] = parameterType.InstantiateDefault(false);
			}
        ...
        /// </summary>
        /// <param name="type">The <see cref="Type"/> to weigh the constructors of.</param>
        /// <param name="flags">The <see cref="BindingFlags"/> to search for the constructors; <see cref="BindingFlags.Default"/> means only find the public ones.</param>
        /// <returns>The most ideal <see cref="ConstructorInfo"/> based on the scoring system.</returns>
		...
		{
			ConstructorInfo[] ctors = flags == BindingFlags.Default ? 
			...
			var filteredCtors = new List<ConstructorInfo>(capacity: ctors.Length);
			
			for (var i = 0; i < ctors.Length; i++)
			{
				if (IsUnmanagedCtor(ctors[i]))
				{
					continue;
				}

                if (HasNonDefaultContractTypes(ctors[i]))
                {
                    continue;
                }
				
				filteredCtors.Add(ctors[i]);
			}

			if (filteredCtors.Count < 1)
			{
				return null;
			}

			ConstructorInfo currentCtor = filteredCtors[0];
			int currentCtorScore = GetCtorScore(currentCtor);
			
			for (var i = 1; i < filteredCtors.Count; i++)
			{
				ConstructorInfo nextCtor = filteredCtors[i];
				int nextCtorScore = GetCtorScore(nextCtor);

				if (nextCtorScore <= currentCtorScore)
				{
					continue;
				}

				currentCtor = nextCtor;
				currentCtorScore = nextCtorScore;
			}
        ...
        /// <summary>
        /// Checks if a given <see cref="ConstructorInfo"/> is unmanaged, by checking if it contains any <see cref="Type.IsPointer"/>, <see cref="Type.IsByRef"/> or <see cref="Type.IsMarshalByRef"/> parameters.
        /// </summary>
        /// <param name="ctor">The <see cref="ConstructorInfo"/> to validate.</param>
        /// <returns><c>true</c> if the <see cref="ConstructorInfo"/> is unmanaged; otherwise <c>false</c>.</returns>
		...
		{
			const int SCORE_BASE = 4;
            const int LENGTH_SCORE = -3;
            const int VALUE_TYPE_SCORE = 5;
            const int REFERENCE_TYPE_SCORE = -60;
            const int DEFAULT_VALUE_SCORE = 30;
            const int DEFAULT_VALUE_REFERENCE_SCORE = 50;
            const int ALL_DEFAULT_VALUES_BONUS = 10_000;
            const int EMPTY_CTOR_BONUS = 50_000;
			...
			for (var i = 0; i < parameters.Length; i++)
            {
                ...
                score += isValueType ? VALUE_TYPE_SCORE : REFERENCE_TYPE_SCORE;
                ...
                score += isValueType ? DEFAULT_VALUE_SCORE : DEFAULT_VALUE_REFERENCE_SCORE;
                defaultValueCount++;
            }

            // NOTE: Highly prefer constructors that can be created using purely default values.
            if (defaultValueCount == parameters.Length)
            {
                score += ALL_DEFAULT_VALUES_BONUS;
            }
