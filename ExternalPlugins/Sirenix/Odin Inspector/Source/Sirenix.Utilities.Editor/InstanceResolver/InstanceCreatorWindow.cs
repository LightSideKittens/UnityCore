...
#pragma warning disable
    ...
    internal class InstanceCreatorWindow : EditorWindow
    {
        ...
        private TypeTreeNode GetTypeTree(Type type)
        {
            ...
            {
                var typeFlag = AssemblyUtilities.GetAssemblyCategory(type.Assembly);
                ...
                rootNode = typeTrees[type] = new TypeTreeNode((Type)null)
                {
                    ...
                    .GetAllAssemblies()
                    .Select(x => new { assembly = x, flag = AssemblyUtilities.GetAssemblyCategory(x) })
                    .GroupBy(a => a.flag)
        ...
        private class TypeTreeNode
        {
            ...
            {
                this.AssemblyCategory = assemblyTypeFlags;
            }
            ...
            public void PrependType(Type type)
            {
                ...
                var flag = AssemblyUtilities.GetAssemblyCategory(specialType.Assembly);
                var node = this.ChildNodes.FirstOrDefault(x => x.AssemblyCategory == flag);
                if (node == null)
                {
            ...
            public void Initialize(InstanceCreatorWindow window, TypeTreeNode parent)
            {
                ...
                this.isAssemblyFlagNode = this.AssemblyCategory != AssemblyCategory.Unknown;
                this.labelName = null;
                ...
                else if (this.isAssemblyFlagNode)
                {
                    if (this.ChildNodes != null && this.ChildNodes.Count > 0)
                    {
                        this.labelName = this.AssemblyCategory.ToString().SplitPascalCase();
                        this.label = new GUIContent(this.labelName);
                    }
                }
